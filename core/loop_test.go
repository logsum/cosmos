...
func TestTrackerAdjustTokens(t *testing.T) {
	// Test AdjustTokens replaces existing accumulations
	tracker := NewTracker(nil, nil)
	...
	// Record initial usage from multiple sources
	tracker.Record(model, provider.Usage{InputTokens: 100, OutputTokens: 50}, SourcePrompt)
	tracker.Record(model, provider.Usage{InputTokens: 200, OutputTokens: 100}, Source("tool"))

	// Verify initial state
	snap := tracker.Snapshot()
	if snap.TotalInputTokens != 300 || snap.TotalOutputTokens != 150 {
		t.Errorf("initial tokens = %d input, %d output, want 300 input, 150 output",
			snap.TotalInputTokens, snap.TotalOutputTokens)
	}

	// Adjust tokens (after compaction)
	tracker.AdjustTokens("test-model", 80, 40)

	// Verify adjusted state
	snap = tracker.Snapshot()
	if snap.TotalInputTokens != 80 || snap.TotalOutputTokens != 40 {
		t.Errorf("adjusted tokens = %d input, %d output, want 80 input, 40 output",
			snap.TotalInputTokens, snap.TotalOutputTokens)
	}

	// Verify only one source remains ("compacted")
	if len(snap.Models) != 1 {
		t.Fatalf("model count = %d, want 1", len(snap.Models))
	}
	if len(snap.Models[0].Sources) != 1 {
		t.Errorf("source count = %d, want 1 (compacted)", len(snap.Models[0].Sources))
	}
}

func TestTrackerAdjustNonExistentModel(t *testing.T) {
	// Test AdjustTokens on model not yet tracked (no-op)
	tracker := NewTracker(nil, nil)

	// Adjust non-existent model (should not panic)
	tracker.AdjustTokens("nonexistent-model", 100, 50)

	// Verify no models tracked
	snap := tracker.Snapshot()
	if len(snap.Models) != 0 {
		t.Errorf("model count = %d, want 0", len(snap.Models))
	}

	// Verify subsequent Record works normally
	model := provider.ModelInfo{
		ID:              "test-model",
		Name:            "Test Model",
		ContextWindow:   1000,
	...
	tracker.Record(model, provider.Usage{InputTokens: 10, OutputTokens: 5}, SourcePrompt)

	snap = tracker.Snapshot()
	if snap.TotalInputTokens != 10 || snap.TotalOutputTokens != 5 {
		t.Errorf("tokens = %d input, %d output, want 10 input, 5 output",
			snap.TotalInputTokens, snap.TotalOutputTokens)
	}
}
